...
module sync_conv #(
    ...
    input logic [PRECISION-1:0] in_features_a [INPUT_DIM-INPUT_PARAMETER-1:0],
    input logic [PRECISION-1:0] in_features_b [INPUT_DIM-INPUT_PARAMETER-1:0],
    ...
    logic [PRECISION-1:0] temp_in_features_a [INPUT_DIM-INPUT_PARAMETER-1:0];
    logic [PRECISION-1:0] temp_in_features_b [INPUT_DIM-INPUT_PARAMETER-1:0];
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (in_event.valid) begin
                temp_event <= in_event;
                temp_edges <= in_edges;
                temp_in_features_a <= in_features_a;
                temp_in_features_b <= in_features_b;
                counter <= 0;
            end
    ...
    // porta
    always @(posedge clk) begin
        for (int i = 0; i < INPUT_DIM-2; i++) begin
            feature_mat_a[i] <= $signed(temp_in_features_a[i]);
        end
        feature_mat_a[INPUT_DIM-2] <= ((counter == MEMORY_OPS_NUM-1) ? '0 : (temp_edges[counter].df )) >>> (F_WIDTH-PRECISION);
        feature_mat_a[INPUT_DIM-1] <= ((counter == MEMORY_OPS_NUM-1) ? '0 : (temp_edges[counter].dt )) >>> (T_WIDTH-PRECISION);
    end
    ...
    always @(posedge clk) begin
        for (int i = 0; i < INPUT_DIM-2; i++) begin
            feature_mat_b[i] <= (counter == MEMORY_OPS_NUM-1) ? '0 : temp_in_features_a[i];
        end
        feature_mat_b[INPUT_DIM-2] <= ((counter == MEMORY_OPS_NUM-1) ? '0 : (temp_edges[counter + MEMORY_OPS_NUM-1].df)) >>> (F_WIDTH-PRECISION);
        feature_mat_b[INPUT_DIM-1] <= ((counter == MEMORY_OPS_NUM-1) ? '0 : (temp_edges[counter + MEMORY_OPS_NUM-1].dt)) >>> (T_WIDTH-PRECISION);
    end                               
