...
module convolution #(
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (counter_reg == F_RADIUS && counter == F_RADIUS && state == CONV && outdim_counter == (OUTPUT_DIM)-1) begin
                state <= IDLE;
                start <= '1;
            end
            if (state == IDLE) begin
    ...
    logic signed [PRECISION_OUT:0] single_weight_reg [INPUT_DIM+1:0];
    ...
    generate
        for (w = 0; w < INPUT_DIM+2; w++) begin : weights_assign
            always @(posedge clk) begin
                single_weight_reg[w] <= weight_mem[(((PRECISION_OUT)*(w+1))-1)+32 : ((PRECISION_OUT)*w)+32] - ZERO_POINT_WEIGHT;
            end
        end
    endgenerate
