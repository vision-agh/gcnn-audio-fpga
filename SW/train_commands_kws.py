...
def main():
    ...
    wandb_logger = WandbLogger(project='audio_event', name=f'commands')
    ...
    wandb_logger = WandbLogger(project='audio_event', name=f'commands_qat')
    wandb_logger.watch(model)
    lr_monitor = LearningRateMonitor(logging_interval='step')

    checkpoint_callback = ModelCheckpoint(
        dirpath='checkpoints',
        filename='best_model_calibrated_commands',
        monitor='val_acc',
        mode='max',
        save_top_k=1
    )
