...
def main():
    ...
    wandb_logger = WandbLogger(project='audio_event', name=f'digits_rec')
    ...
    wandb_logger = WandbLogger(project='audio_event', name=f'digits_rec_float_qat')
    wandb_logger.watch(model)
    lr_monitor = LearningRateMonitor(logging_interval='step')

    checkpoint_callback = ModelCheckpoint(
        dirpath='checkpoints',
        filename='digits_rec_calibrated',
        monitor='val_acc',
        mode='max',
        save_top_k=1
    )
